...
void alpaka_copy(TSource& source, Ttarget& target, Textend& extend)
{
    ...
    alpaka::memcpy(
        ...
        target,
        source,
        extend); // big buffer on device-->copy--> small buffer on device[1] --> host buffer
    alpaka::wait(queue);
};
...
public:
    ...
    {
        bufHost = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::PlatformCpu{}, 0), this->extent1D));
    ...
    Vector_h(Vector_h<T>& source) : extent1D(Vec(source.size())), size_param(source.size())
    {
        bufHost = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::PlatformCpu{}, 0), this->extent1D));
        this->pHost = alpaka::getPtrNative(*this->bufHost);
    ...
    void resize(TIDX N)
    {
        ...
        this->extent1D = Vec(N);
        this->bufHost = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::PlatformCpu{}, 0), this->extent1D));
        this->pHost = alpaka::getPtrNative(*this->bufHost);
    }
...
public:
    ...
    {
        this->bufAcc = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::Platform<Acc>{}, 0), this->extent1D));
    ...
    Vector_d(Vector_h<T>& source) : size_param(source.size()), extent1D(Vec(source.size()))
    {
        this->bufAcc = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::Platform<Acc>{}, 0), this->extent1D));
        this->pAcc = alpaka::getPtrNative(*this->bufAcc);
    ...
    Vector_d(Vector_d<T>& source) : size_param(source.size()), extent1D(Vec(source.size()))
    {
        this->bufAcc = std::make_shared<Buffer>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::Platform<Acc>{}, 0), this->extent1D));
        this->pAcc = alpaka::getPtrNative(*this->bufAcc);
    ...
    void fill(T v)
    {
        ...
        std::shared_ptr<BufHost> hostbuf = std::make_shared<BufHost>(
            alpaka::allocBuf<T, Idx>(alpaka::getDevByIdx(alpaka::PlatformCpu{}, 0), this->extent1D));
...
public:
    ...
    inline Vector_h(Vector_h<T> const& a) : thrust::host_vector<T>(a)
    {
    }

    inline Vector_h(Vector_d<T> const& a) : thrust::host_vector<T>(a)
    {
    }

    template<typename OtherVector>
    inline void copy(OtherVector const& a)
    {
        this->assign(a.begin(), a.end());
    }

    inline Vector_h<T>& operator=(Vector_h<T> const& a)
    {
        copy(a);
        return *this;
    }

    inline Vector_h<T>& operator=(Vector_d<T> const& a)
    {
        copy(a);
        return *this;
    }
...
public:
    ...
    inline Vector_d(Vector_d<T> const& a) : thrust::device_vector<T>(a)
    {
    }

    inline Vector_d(Vector_h<T> const& a) : thrust::device_vector<T>(a)
    {
    }
    ...
    inline Vector_d<T>& operator=(Vector_d<T> const& a)
    {
        copy(a);
        return *this;
    }

    inline Vector_d<T>& operator=(Vector_h<T> const& a)
    {
        copy(a);
        return *this;
    }
